...
文件树里不止 Markdown/HTML——点开一个 `.py` / `.ts` / `.json` / `.xml` / `.rs` / `.go` 等**代码文件**,会在同一编辑器里以**按扩展名自动识别的语法高亮**打开,可**直接编辑**（不再只是只读预览）——和 Markdown 一样**自动保存**（停顿约 2 秒即落盘,也可随时 `⌘S` 立即保存）,适合在笔记旁顺手维护脚本、配置、示例代码。

- 代码文件**不进入知识库编译**（不生成 Wiki 页）,顶栏也不显示导出/重摄取按钮——它只是磁盘上的一个普通文本文件。
- **图片**（`.png` / `.svg`…）显示为图片查看器,**二进制**（`.zip` / `.pdf`…）显示为占位说明,两者保持只读。
- **不是合法 UTF-8** 的文件不会被当文本打开,只读展示,避免误改损坏字节。
