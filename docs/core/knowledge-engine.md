...
有一堆现成笔记想批量入库?在知识模式下**右键主知识空间里的任意目录 → 「ingest 整个目录」**,引擎会把该目录下的笔记逐个编译进 Wiki,不用一篇篇手动开。

- 摄入进行时,目录行显示**进度徽章**,可展开**队列面板**看每个文件的处理状态。
- 觉得跑太久 / 选错目录?随时**终止**摄入——已完成的保留,未开始的取消。
- 目录级 ingest 仅对**主知识空间**生效([外部挂载](/core/knowledge-engine#外部挂载-迁移与外部目录)不进索引,故不提供此菜单)。

![目录批量摄入进度：目录行进度徽章展开的队列面板，显示处理中的文件、等待队列与「终止全部」](/screenshots/ingest-directory.jpg)
...
```markdown
今天决定把 KnowMe 的外部挂载先设计成「可读写但不进索引」。
原因是用户可能挂载 Obsidian 仓库，我们不应该默认把旧库全部编译进 Wiki。
后续如果要支持选择性索引，再做显式开关。
```
...
这些候选结果会先进入 Review queue，只有你确认后才进入正式知识库。
...
类型由 LLM 编译自动推断(也可由笔记 frontmatter `type:` 指定)。不同类型在界面里用不同强调色区分。自定义 Wiki 类型属于 Pro 能力,见 [价格](https://useknowme.com/#pricing)。
...
队列里若有**诊断条目**(如某次摄入的告警 / 错误),可**点开就地展开看全文与结构化详情**,不必跳去别处翻日志——一眼看清哪篇笔记为什么没编译成功。
...
> 脉络把**结构性引用**(谁链到我)和 **LLM 语义关系**(谁和我相关)放在同一面板,帮你一眼看清一个词条的上下文。未解析到目标的关系会以不可跳转状态展示,避免假链接。
...
| **脉络** | Wiki 的[关系脉络](#脉络-右栏「脉络」):语义关系 + 反向引用(仅 Wiki 页) |
...
右栏面板的完整标签结构(笔记页 `交流 / 评论 / 相关`,Wiki 页 `交流 / 改进 / 脉络`)见 [AI 协作 · 右栏面板](/ai/collaboration#右栏面板-ai-协作都在这里)。
