...
右键任一条消息气泡（微信式），弹出原生菜单：

- **复制** —— 把该消息文本复制到剪贴板。
- **引用** —— 把该消息作为 Markdown 引用块（`> …`）插入输入框，方便追问时带上原文。
- **多选** —— 进入多选态，气泡出现勾选框、底部浮出操作条，可**批量复制 / 批量引用**（按转录顺序合并）；点空白处或 `Esc` 退出。
- **回退到此处**（仅 opencode 会话）—— 调用 opencode 的 `session.revert`，把该消息及其之后的对话连同文件改动一起回退；成功后顶部出现「已回退 · 撤销」提示，点**撤销**即 `unrevert` 恢复。会话正在流式回复时该项禁用。

::: tip 回退是 opencode 官方语义
「回退到此处」走 opencode serve 的 `revert`/`unrevert`，可逆且连带文件改动，与 `/undo` 斜杠命令同源。codex 会话无此能力，菜单不显示该项。
:::
...
右键任一会话行：重命名 / 移出列表 / **彻底删除** / 复刻 / 复制会话 ID / 归档 / 归档并入库。支持多选批量操作。
