...
- **主知识空间** —— 当前知识空间的主笔记目录。换这个目录 = 切换到另一个知识空间(Wiki / 评论 / 数据库整体随之切换)。详见 [知识引擎 · 知识空间](/core/knowledge-engine#知识空间)。
- **外部挂载** —— 把 Obsidian / iCloud / 任意文件夹"借"进知我视野(读写,但**不进知识库索引**)。每个挂载可设可见范围 **全局 / 本空间**(新挂载默认全局):全局挂载在所有知识空间显示,本空间挂载只在当前空间显示。详见 [知识引擎 · 外部挂载](/core/knowledge-engine#外部挂载-迁移与外部目录)。

![配置中心 → 知识空间：主知识空间 + 外部挂载的全局 / 本空间范围](/screenshots/knowledge-space-mounts.jpg)
